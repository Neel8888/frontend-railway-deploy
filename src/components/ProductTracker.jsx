...
function ProductTracker({ url, setUrl }) {
  ...
  const fetchData = async () => {
    setLoading(true);
    try {
      let destUrl = `https://mybackend-deploy-production.up.railway.app/scrape?url=${encodeURIComponent(
        url
      )}`;
      console.log(destUrl);
  ...
  const trackData = async () => {
    setLoading(true);
    try {
      let trackUrl = `https://mybackend-deploy-production.up.railway.app/scrape?url=${encodeURIComponent(
        url
      )}`;
